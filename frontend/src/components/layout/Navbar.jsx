...
const CATEGORIES = [
  { label: 'Концерты', slug: 'concerts', icon: Music,   color: '#8B5CF6' },
  { label: 'Театр',    slug: 'theatre',  icon: Theater,  color: '#EC4899' },
  { label: 'Спорт',    slug: 'sport',    icon: Trophy,   color: '#10B981' },
  { label: 'Стендап',  slug: 'standup',  icon: Mic2,     color: '#F59E0B' },
  { label: 'Выставки', slug: 'exhibition',icon: Image,   color: '#06B6D4' },
  { label: 'Кино',     slug: 'cinema',   icon: Film,     color: '#EF4444' },
];
...
export default function Navbar() {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQ.trim()) { navigate(`/events?search=${encodeURIComponent(searchQ.trim())}`); setSearchOpen(false); setSearchQ(''); }
  };
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            <div className="relative" onMouseEnter={() => setCatMenu(true)} onMouseLeave={() => setCatMenu(false)}>
              <button className={cn('btn-ghost text-sm px-4 flex items-center gap-1', catMenu && 'text-brand-400')}>
                Категории <ChevronDown className={cn('w-3.5 h-3.5 transition-transform duration-200', catMenu && 'rotate-180')} />
              </button>
              <AnimatePresence>
                {catMenu && (
                  <motion.div initial={{ opacity: 0, y: 8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                    ...
                    {CATEGORIES.map(cat => (
                      <Link key={cat.slug} to={`/events?category=${cat.slug}`}
                        ...
                        <span className="text-sm text-white/70 group-hover/cat:text-white transition-colors">{cat.label}</span>
                      </Link>
                    ))}
                  </motion.div>
                )}
          ...
          <div className="flex items-center gap-1.5">
              ...
              {searchOpen ? (
                <motion.form key="form" initial={{ width: 40, opacity: 0 }} animate={{ width: 220, opacity: 1 }}
                  ...
                  <input ref={searchRef} value={searchQ} onChange={e => setSearchQ(e.target.value)}
                    placeholder="Поиск..." className="bg-transparent text-sm text-white placeholder-white/30 outline-none w-full" />
            ...
            {isAuthenticated ? (
                  ...
                  {userMenu && (
                    <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                      ...
                      {[
                        ...
                        { to: '/orders',   icon: Ticket,        label: 'Мои билеты' },
                        { to: '/favorites',icon: Heart,         label: 'Избранное' },
                        ...(user?.role === 'ADMIN' ? [{ to: '/admin', icon: LayoutDashboard, label: 'Админ панель' }] : []),
                      ].map(item => (
              ...
              <div className="flex items-center gap-2">
                <Link to="/login" className="btn-ghost text-sm px-4 py-2 hidden sm:block">Войти</Link>
                <Link to="/register">
          ...
          {mobileOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
              ...
              <div className="px-4 py-4 space-y-1">
                <Link to="/events" className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/5 text-sm font-medium">Афиша</Link>
                {CATEGORIES.map(cat => (
                  <Link key={cat.slug} to={`/events?category=${cat.slug}`}
                ...
                {!isAuthenticated && (
                  <div className="flex gap-2 pt-3 border-t border-white/10">
                    <Link to="/login" className="btn-secondary flex-1 text-center text-sm py-2.5">Войти</Link>
                    <Link to="/register" className="btn-primary flex-1 text-center text-sm py-2.5">Регистрация</Link>
                  </div>
                )}
                {isAuthenticated && (
                  <div className="pt-3 border-t border-white/10 space-y-1">
                    <Link to="/profile"   className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/5 text-sm"><User className="w-4 h-4 text-brand-400" /> Профиль</Link>
                    <Link to="/orders"    className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/5 text-sm"><Ticket className="w-4 h-4 text-brand-400" /> Мои билеты</Link>
                    <Link to="/favorites" className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/5 text-sm"><Heart className="w-4 h-4 text-brand-400" /> Избранное</Link>
