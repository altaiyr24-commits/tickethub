...
export default function AdminUsers() {
  ...
  const toggleRole = async (user) => {
    ...
    try {
      ...
      toast.success(`${t('common.save')} → ${newRole}`);
  ...
  return (
    <>
      <Helmet><title>{t('admin.users.title')} — Admin</title></Helmet>
      <div className="p-8">
        <div className="mb-8">
          <h1 className="font-display text-3xl font-black mb-1">{t('admin.users.title')}</h1>
          <p className="text-white/40">{users.length} {t('common.seats')}</p>
        </div>

        <div className="relative mb-6">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/30" />
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder={t('admin.users.searchPlaceholder')} className="input-field pl-12" />
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  {[t('admin.users.columns.user'), 'Email', t('admin.users.columns.role'), t('admin.users.columns.registered'), t('admin.users.columns.actions')].map(h => (
              ...
              <tbody>
                {filtered.map((user, i) => (
                  <motion.tr key={user.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                    ...
                    <td className="px-4 py-3 text-sm text-white/40">{formatDate(user.created_at || user.createdAt)}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => toggleRole(user)} title={user.role === 'ADMIN' ? t('admin.users.makeUser') : t('admin.users.makeAdmin')}
